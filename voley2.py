...
from clases_voley import Equipos
...
m2.maximize(m2.sum(m2.sum(m2.sum(equipo.preferencia_local_en_dia(t) * partido[equipo, fecha, t] for fecha in fechas) for t in dias for equipo in Equipos))
...
m2.add_constraints(partido[i,j,t] == partido[i,j+1,t+2] for i in Equipos for j in fechas[:-1] for t in dias[:cantidad_de_dias-2] if (i.esta_de_viaje_en_fecha_y__en_siguiente(j)))
#descanso de al menos dos dias antes de un viaje
m2.add_constraints(partido[i,j-1,t-2] + partido[i,j-1,t-1] + partido[i,j,t] <= 1 + b[i,j,t] for i in Equipos for j in fechas[1:] if i.comienza_viaje_en_fecha(j) for t in dias[2:])
#descanso de al menos dos dias despues de un viaje
m2.add_constraints(partido[i,j+1,t+2] + partido[i,j+1,t+1] + partido[i,j,t] <= 1 + c[i,j,t] for i in Equipos for j in fechas[:-1] if i.finaliza_viaje_en_fecha(j) for t in dias[:cantidad_de_dias - 2])
#Se corresponden los partidos de los equipos que juegan en contra
m2.add_constraints(partido[equipo1,fecha1,t] == partido[equipo2,fecha2,t] for equipo1 in Equipos for equipo2 in Equipos if (equipo1!=equipo2) for fecha1 in fechas for fecha2 in fechas if equipo1.juega_contra_en_fechas(equipo2, fecha1, fecha2) for t in dias)
#Dias televisados
...
m2.add_constraints(partido[i,j,t] + partido[i,j+2,t+4] <= 1 for i in Equipos for j in fechas[:cantidad_de_fechas-2] if i.finaliza_viaje_en_fecha(j) for t in dias[:cantidad_de_dias - 4])
m2.add_constraints(partido[i,j,t] + partido[i,j-2,t-4] <= 1 for i in Equipos for j in fechas[2:] if i.comienza_viaje_en_fecha(j) for t in dias[4:])
...
m2.add_constraints(partido[i,j,t-1] == 0 for i in Equipos_libertadores1 for j in fechas for t in libertadores1 if i.esta_de_viaje_en_fecha_y__en_siguiente(j))
m2.add_constraints(partido[i,j,t-1] == 0 for i in Equipos_libertadores2 for j in fechas for t in libertadores2 if i.esta_de_viaje_en_fecha_y__en_siguiente(j))
m2.add_constraints(partido[i,j,t-1] == 0 for i in Equipos_copaaclav for j in fechas for t in copaaclav if i.esta_de_viaje_en_fecha_y__en_siguiente(j))
m2.add_constraints(partido[i,j,t-1] == 0 for i in Equipos_lvasocial for j in fechas for t in lvasocial if i.esta_de_viaje_en_fecha_y__en_siguiente(j))
m2.add_constraints(partido[i,j,t-1] == 0 for i in Equipos_desafio for j in fechas for t in desafio if i.esta_de_viaje_en_fecha_y__en_siguiente(j))
m2.add_constraints(partido[i,j,t-1] == 0 for i in Equipos_sudamericano for j in fechas for t in sudamericano if i.esta_de_viaje_en_fecha_y__en_siguiente(j))
######Torneos#######
